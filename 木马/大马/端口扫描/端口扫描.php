...
= getHeader("Location");
    ...
    if(strpos(getHeader("Location"),'http://') == false){
      ...
      $location
= getHost($url).$location;

    }

        return
getHtmlContext($location);
...
= $host."/".$v;
    ...
    $csshtml
= "<style>".file_get_contents($cssurl)."</style>";
...
= getHost($url);
    ...
    echo
getCss($host,getHtmlContext($url));
